 ...
 * 
 * @plugindesc [0.2.5] Haya Map Editor
 * 
    ...
    class Scene_Editor extends Scene_Base {
            ...
            this.button.save = new Haya.GUI.Button({
                ...
                action: () => {
                    ...
                    Haya.Map.current.sprite.forEach((sprite) => {
                        ...
                        sprite._layerInfo.anchor_x = sprite.anchor.x;
                        sprite._layerInfo.anchor_y = sprite.anchor.y;
        ...
        updatePivot() {
            ...
            if (Haya.Mouse.y.isBetween(0, Graphics.width) && (Haya.Mouse.y.isBetween(0, 16))) {
                if ((this.toolbar === false)) {
                    this.camera.y = Haya.DMath.fdecrease(this.camera.y, 0, (Haya.Map.current.height - Graphics.height) + 1, 8)
                }
                ...
                if ((this.toolbar === false)) {
                    this.camera.y = Haya.DMath.fincrease(this.camera.y, 0, (Haya.Map.current.height - Graphics.height) + 1, 8)
                }
        ...
        weditSound() {
            if ($.editor.wedit === "position") {
                ...
                // editing
                if (TouchInput.isLongPressed()) { // isPressed
                    $.editor.target.haya.pos.x = Haya.Mouse.x + this.display.x;
                    $.editor.target.haya.pos.y = Haya.Mouse.y + this.display.y;
                ...
                if (TouchInput.wheelY >= 20) {
                    ...
                    $.editor.target.haya._data.coneInnerAngle = Haya.DMath.fdecrease($.editor.target.haya._data.coneInnerAngle, 0, 10, 1, "alt", 15);
                    this.changePannerAttr();
                    this.gui.editor.button.coneInnerAngle.text(`coneInnerAngle: ${$.editor.target.haya._data.coneInnerAngle}`)
                }
                ...
                if (TouchInput.wheelY >= 20) {
                    ...
                    $.editor.target.haya._data.coneOuterAngle = Haya.DMath.fdecrease($.editor.target.haya._data.coneOuterAngle, 0, 10, 1, "alt", 15);
                    this.changePannerAttr();
                    this.gui.editor.button.coneOuterAngle.text(`coneOuterAngle: ${$.editor.target.haya._data.coneOuterAngle}`)
                }
        ...
        weditSprite() {
            if ($.editor.wedit === "alpha") {
                ...
                $.editor.target.anchor.x = Haya.DMath.wheelID(
                    $.editor.target.anchor.x,
                    0.0, 1, 0.1,
                    (current) => { this.gui.editor.button.anchorX.text(`Anchor X: ${Haya.DMath.float(current)}`) },
                    "alt", 0.5
                )
            } else if ($.editor.wedit === "anchorY") {
                $.editor.target.anchor.y = Haya.DMath.wheelID(
                    $.editor.target.anchor.y,
                    0.0, 1, 0.1,
                    (current) => { this.gui.editor.button.anchorY.text(`Anchor Y: ${Haya.DMath.float(current)}`) },
                    "alt", 0.5
                )
            }
        }
