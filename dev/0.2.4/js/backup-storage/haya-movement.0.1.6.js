 ...
 * 
 * @plugindesc [0.1.6] Haya Movement | Organic Movement 
 * 
 * @help
 ...
 * ======================================================================
 * LOG <Version - Change when released>
 * 0.1.0
 *  [Plugin Create at 25/09/20118]
 *  -                                   
 * ======================================================================
 */
...
(function ($) {
    ...
    $.createCollision = function (system, kind, setup) {
        ...
        if (kind === "polygon") {
            let polygon = system.createPolygon(
                setup.x || 0,
                setup.y || 0,
                [...setup.points],
                setup.angle || 0,
            ...
            polygon.cachePoints = [...setup.points];
            return polygon;
        } else if (kind === "circle") {
...
(function ($) {
    'use strict';
    // ============================================================================
    $.setup = Haya.FileIO.json(Haya.FileIO.local("data/immersion/movement.json"));
    $.data = {};
    $.bounce = 0.9;
     ...
     */
    function setStateBody(data, parent) {
        var body = null;
        ...
        if (data.type === "circle") {
            ...
            data.radius = Haya.Utils.Object.hasProperty(data, "radius", 18);
    ...
    Game_CharacterBase.prototype.initMembers = function () {
        ...
        this.animation = {
            lastState: {},
            state: { jumping: 0, dashing: 0, moving: 0, idle: 0 },
            idling: true,
            get: "idle",
            override: -1,
            x: -1, y: -1,
        ...
        if (Haya.Utils.isObject(this.animation.data)) {
            ...
            if (this.animation.data.hasOwnProperty("collision")) {
                if (this.animation.collisionState !== this._collision.state) {
                    this._collision.body = setStateBody(
                    ...
                    //print("refresh collision body", this._collision.body)
                }
    ...
    Game_Character.prototype.animationStateUpdate = function () {
        ...
        } else { this.animation.state.moving = 0; }
        // idle?
        if (this.animation.idling) {
            this.animation.state.idle++;
        } else { this.animation.state.idle = 0; }
    }
    ...
    Sprite_Character.prototype.setCharacterBitmap = function () {
        if (this._characterName === "" || (Haya.FileIO.exist("img/characters/" + this._characterName + ".png")) === false) {
            this.bitmap = ImageManager.loadCharacter(null);
            this._data = Haya.FileIO.json(Haya.FileIO.local("img/characters/default.json"))
            ...
            } else { characterSetup = this._characterName + ".json"; }

            if (Haya.FileIO.exist("img/characters/" + characterSetup)) {
                this._data = Haya.FileIO.json(Haya.FileIO.local("img/characters/" + characterSetup));
    ...
    Scene_Map.prototype.playerCollisionOriginal = function () {
        // check out all available collisions
        const player_potentials = $gamePlayer._collision.body.potentials();
        //$.data.collision.base.boucing
        //$.data.collision.base.space
        if ($gamePlayer._collision._x === -1) $gamePlayer._collision._x = $gamePlayer._x;
        if ($gamePlayer._collision._y === -1) $gamePlayer._collision._y = $gamePlayer._y;
        ...
        for (const wall of player_potentials) {
            ...
            if ($gamePlayer._collision.body.collides(wall, $.result)) {
                ...
                // speed up
                $gamePlayer._moveSpeed = ($gamePlayer._cacheMoveSpeed * $.result.overlap);

                print($gamePlayer._collision._x, $gamePlayer._collision._y);

                // the difference between the original position with the position when pushed against the wall
                $gamePlayer._x -= ($gamePlayer._x - $gamePlayer._collision._x) + ($.result.overlap_x * 0.075)
                $gamePlayer._y -= ($gamePlayer._y - $gamePlayer._collision._y) + ($.result.overlap_y * 0.075)
        ...
        // the position before colliding;
        $gamePlayer._collision._x = $gamePlayer._x;
        $gamePlayer._collision._y = $gamePlayer._y;
        print($gamePlayer._collision._x, $gamePlayer._collision._y);
