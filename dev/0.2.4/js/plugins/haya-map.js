...
(function ($) {
    ...
    Route.group(instance => {
        ...
        // // shader files
        // instance.set('js', Routes.Game.get('js').path)
    ...
    class HowlSound {
        ...
        desmute() {
            setTimeout(() => {
                ...
                this.sound._muted = false;
                $.sound_setPannerAttr(this.sound)
            }, 150)
        }

        isOkay() {
            return ($.Time.period(this.sound.editor.time)) === true ||
                (this.sound.editor.switch > 1 && $gameSwitches.value(this.sound.editor.switch) === true)
        }

        updt_collision() {
            return $gamePlayer._collision.body.collides(
                this.sound.editor.collision,
                Haya.Movement.result
            )
        }

        update() {

            if (this.sound.editor.collision !== null) {
                if (this.updt_collision() === false) {
                    return
                }
            }
            ...
            if (this.sound.playing()) {
                // check if the period is okay | or the switch
                if (!this.isOkay()) {
    ...
    $.sound_setPannerAttr = function (sound) {
        console.log(sound.editor._data);
...
function ($) {
    ...
    Loader_Map.prototype.finish = function () {
        ...
        $.current.sound.element.map(sounds => sounds._self.desmute())
...
function ($) {
    ...
    Pixi_Light.prototype.configure = function () {
        ...
        if (Array.isArray(this._setup.target)) {
            this._setup.target = new Haya.DMath.Vector2D(this._setup.target[0], this._setup.target[1])
        }
...
function ($) {
    ...
    Sprite_Map.prototype.initialize = async function () {
        ...
        // control the access to the weather objects
        this.weather = {
            source: {},
            element: []
        };
    ...
    Sprite_Map.prototype.createCharacter = function () {
        ...
        $gameMap.events().forEach((event) => {
            ...
            let _event = new Haya.Map.Hayaset_Character(event);
        ...
        this._characters.forEach((value, index) => {
            ...
            value.character = value.children[0]._character;
    ...
    Sprite_Map.prototype.createParticle = function () {
        Object.keys($.current.particle).map((key) => {
            let element = $.current.particle[key]

            //hash.setupFilename = item.setup;
            // let _particle = Haya.Particle.manager.add(item.setup, item.textures, hash)
            element.setup.stage = this.sprite;
            let _particle = Haya.Particle.manager.add({
    ...
    Sprite_Map.prototype.createFilter = function () {
        Object.keys($.current.filter).map((key) => {
            var element = $.current.filter[key]

            if (Haya.Filters.manager.has(element.filter)) {

                var targets = []

                // // targets
                // element.targets.map(target => {
                //     Haya.Filters.from(target)
                // })



                // filter
                var filter = Haya.Filters.manager.add({
                ...
                // targets
                Haya.Filters.from(filter)

                this.filter.source[filter.name] = filter;
                this.filter.element.push(filter)
            }
        })
    }
    /**
    ...
    Sprite_Map.prototype.createWeather = function () {
        Object.keys($.current.weather).map((key) => {
            var element = $.current.weather[key]

            if (Haya.Filters.manager.has(element.weather)) {
                ...
                // filter
                var weather = Haya.Weather.manager.add({
                    filter: element.weather,
                    setup: Object.assign({
                        targets
                    }, element.setup)
                })
                ...
                // targets
                Haya.Weather.from(weather)

                this.weather.source[weather.name] = weather;
                this.weather.element.push(weather)
            }
        })
    }
    /**
    ...
    Sprite_Map.prototype.updateCharacters = function () {
        ...
        for (let index = 0; index < this._characters.length; index++) {
            ...
            if (element) {
                ...
                this.updateCharacters_compareChildOrder(element, index);
            }
        }
    }

    /**
     * @function updateCharacters_compareChildOrder
     * It compares the position of the body of character, then reorder it
     */
    Sprite_Map.prototype.updateCharacters_compareChildOrder = function (current, index) {

        const next = Haya.Utils.Array.next(this._characters, index);

        //console.log(next, 'next');
        
        if (Haya.Utils.invalid(next)) return;

        if (current.character.isCollision() && next.character.isCollision()) {
            //current.parent.swapChildren(next, current)
            // y position
            // if (current.character._collision.body.y > next.character._collision.body.y) {
            //     current.parent.swapChildren(next, current)
            // }
            if (Haya.Collision.ysize(current.character._collision.body) >= Haya.Collision.ysize(next.character._collision.body)) {
                //current.parent.swapChildren(current, next)
                if (current.parent.getChildIndex(current) > current.parent.getChildIndex(next) ) {
                    return;
                } else {
                    current.parent.swapChildren(current, next)
                }
        ...
        //     if (a.z !== b.z) {
        //         return a.z - b.z 
        //     } else if (a.character.isCollision() && b.character.isCollision()) {

        //         // if (a.character._collision.body.y !== b.character._collision.body.y) {
        //         //     return a.character._collision.body.y - b.character._collision.body.y;
        //         // }
        //         if (a.y !== b.y) {
        //                     return a.y - b.y;
        //         }
     ...
     * Update the filters
     */
    Sprite_Map.prototype.updateFilters = function () {

        Haya.Filters.manager.update()
    }
     ...
     */
    Sprite_Map.prototype.addParticle = function ({
        config,
        setup,
        textures
    }) {

        setup.stage = this.sprite;
    ...
    Sprite_Map.prototype.removeParticle = function (name) {
        if (this.particle.source.hasOwnProperty(name)) {
            ...
            Haya.Particle.manager.remove(this.particle.source[name].id)
            var copy = []
    ...
    Sprite_Map.prototype.addSound = function (data) {
        ...
        setTimeout(() => {
            $.current.sound.source[sound.editor.name]._self.desmute()
        })
    }
    /**
     * @method removeSound
     ...
     */
    Sprite_Map.prototype.addWeather = function (element) {
        // filter
        var weather = Haya.Weather.manager.add({
     ...
     */
    Sprite_Map.prototype.removeWeather = function (name) {
        if (this.weather.source.hasOwnProperty(name)) {
            //this.particle.source[name].parent.removeChild(this.particle.source[name]);
            Haya.Weather.manager.remove(this.weather.source[name].id)
            var copy = []
            this.weather.element.forEach((value, index) => {
                if (value.name !== name) {
                    copy.push(value)
                }
            })
            this.weather.element = copy
            delete this.weather.source[name];
        }
    }
    /**
     * @method addFilter
     * @description add filters source on map
     */
    Sprite_Map.prototype.addFilter = function (element) {
        var targets = []
     ...
     * @description add filters source on map
     */
    Sprite_Map.prototype.removeFilter = function (name) {
        if (this.filter.source.hasOwnProperty(name)) {
            //this.particle.source[name].parent.removeChild(this.particle.source[name]);
            Haya.Filters.manager.remove(this.filter.source[name].id)
            var copy = []
            this.filter.element.forEach((value, index) => {
                if (value.name !== name) {
                    copy.push(value)
                }
            })
            this.filter.element = copy
            delete this.filter.source[name];
        }
    }
