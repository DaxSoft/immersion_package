 ...
 * 
 * @plugindesc [0.3.2] Haya Map Editor
 * 
...
function ($) {
                    ...
                    Components.notification.Create({
                        ...
                        components: [{
                                ...
                                onclick: function () {
                                    ...
                                    SceneManager._scene.removeCollision(SceneManager._scene.gui.editorCollision.target._name);
                                    $.editor.control = "collision";
                                    SceneManager._scene.refreshEditor();
                                    SceneManager._scene.setupEditor();
                                    this._popup.destroy(true);
                ...
                onclick: () => {
                    Components.notification.Create({
                        ...
                        components: [{
                                ...
                                onclick: function () {
                                    $.editor.wedit = null;
                                    SceneManager._scene._spriteset.removeWeather($.editor.target.name);
                                    $.editor.control = 'weather';
                                    SceneManager._scene.refreshEditor();
                                    SceneManager._scene.setupEditor();
                                    this._popup.destroy(true);
    ...
    class Scene_Editor extends Scene_Base {
        ...
        ucreate_iEdit() {
            ...
            this.gui.editorWeather = new Editor_Weather;
        }
        ...
        updateUI() {
            ...
            // weather editor
            if (this.gui.editorWeather.target !== null && $.editor.control === 'weather-editor') {
                this.gui.editorWeather.update();
            }
        }
        ...
        updateKeys() {
            if (Input.isTriggered('1')) {
                ...
                $gamePlayer.floor = $.editor.collisionFloor;
                SceneManager._scene._spriteset.refreshSprite();
            } else if (Input.isTriggered('t')) {
                new Components.alert.Geral({
                    label: `Period changed to: ${Haya.Map.Time.next()}`
                })
            }
        }
        ...
        particleTargets() {
            ...
            let keys = Object.keys(this._spriteset.particle.source);

            if (keys.length > 0) {
                keys.map(key => {
                    const element = this._spriteset.particle.source[key]
        ...
        setupEditorFilter() {
            ...
            this.gui.editor.el.create.select.onchange = () => {
                var option = this.gui.editor.el.create.data.find(el => el.value === +this.gui.editor.el.create.get())
                ...
                this.gui.editor.window.components = [{
                    ...
                    tag: 'p',
                    innerHTML:  String(option.description)
                }, {
                    component: 'text',
                    label: 'Name:',
        ...
        setupEditorWeather() {
            ...
            this.gui.editor.el.create.select.onchange = () => {
                ...
                this.gui.editor.window.setup.title = `New Weather`
                this.gui.editor.window.refresh();
                ...
                this.gui.editor.window.components = [{
                    component: 'text',
                    label: 'Name:',
                    value: `Weather ` + String(this._spriteset.weather.element.length + 1),
                    onchange: (value) => {
                        $.temp.lpName = value;
                    }
                }, {
                    ...
                    component: 'select',
                    label: 'Range of the Filter:',
                    data: Haya.Weather.range_targets(),
                    onchange: (value) => {
                        $.temp.lpRange = Haya.Weather.range_targets().find(el => el.value === +value).target
                    }
                }, {
                    component: 'button',
                    label: 'Create',
                    ...
                    onclick: () => {
                        ...
                        target.setup = Object.assign({
                            name: ($.temp.lpName || `Weather ` + String(this._spriteset.filter.element.length + 1)),
                            floor: ($.temp.lpFloor || $.editor.collisionFloor),
                            from: ( $.temp.lpRange  || 'map')
                        }, target.setup)


                        SceneManager._scene._spriteset.addWeather(target)
            ...
            this.gui.editor.el.targets = new Components.list.Basic({
                parent: this.gui.editor,
                data: this.weatherTargets(),
                class: 'tree2',
                onclick: (object, element) => {


                    $.editor.control = 'weather-editor'
                    $.editor.target = object.target

                    this.gui.editorWeather.refresh()
                    this.refreshEditor();
                    this.setupEditor();
        ...
        weatherEditor() {
            ...
            this.gui.editorWeather.target = $.editor.target;
            this.gui.editorWeather.create();
        }

        weatherTargets() {
            var data = [];

            this._spriteset.weather.element.map(element => {
                if ($.editor.collisionFloor === "all" || (element.floor === $.editor.collisionFloor)) {
                    data.push({
                        label: `[${element.floor} | ${element.from}]: ${element.name}`,
                        value: data.length,
