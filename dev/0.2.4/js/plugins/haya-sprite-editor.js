...
function ($) {
    ...
    class Editor extends Scene_Base {
        ...
        createEditor() {
            ...
            // collision & body
            this.gui.collision = new Haya.Editor.Collision;
        }
        ...
        editor_collision() {
            ...
            this.gui.collision.onreturn = () => {
                $.controller.to = null;
                SceneManager._scene.refresh_editor();
                SceneManager._scene.controller()
            }

            this.gui.collision.onrefresh = () => {
                $.controller.to = 'collision';
                SceneManager._scene.refresh_editor();
                SceneManager._scene.controller()
            }
        }
        ...
        editor_body() {
            ...
            this.gui.collision.setup({
                ...
                editor: this.gui.editor,
                target: $.controller.target.frame
            })
        ...
        option_invalid() {
            if (
                $.controller.target.config === null
            ) {
                new Components.alert.Geral({
                    label: 'You need to choose a sprite!',
                    time: 2500,
                    type: 'aerror',
                    icon: Routes.Game.plug('gui', 'error.png')
                })
                return true;
            };
            return false;
        ...
        load_sprite() {
            El.Attr(El.create('input', document.body), {
                ...
                if (!Haya.File.exist(`img/characters/${file_config}`)) {
                    Haya.File.wjson($.controller.default, Routes.Game.plug('characters', file_config), false)
            ...
            Components.notification.Create({
                ...
                components: [{
                        ...
                        onclick: function () {
                            ...
                            let filename = $.controller.file.name.replace('.png', '')

                            console.log($.controller.target.config, filename, Routes.Game.plug('characters', filename));
                            ...
                            new Components.alert.Geral({
                                label: 'The modification has been saved!',
                                position: 'bottomRight',
                                type: 'asuccess',
                                icon: Routes.Game.plug('gui', 'success.png'),
                                duration: 3000
                            })
